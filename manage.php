...
$pass = "";
if(isset($_GET["p"])) $pass = $_GET["p"];

if($pass != $admin_psw) {
	 echo("Uhh, you're not allowed here!");
	 exit();
}
...
<div style="width: 70%; display: inline-block; float: left;">
  <div style="float: left; display: inline-block; width: 30%; background-color: #DFDFDF;">
    ...
    <?php
       $c = 0;
       $handle = fopen("H1_entry_list.ini", "r");
       if($handle) {
  ...
  <div style="float: left; display: inline-block; width: 30%; background-color: #DFDFDF;">
    ...
    <?php
       $c = 0;
       $handle = fopen("H2_entry_list.ini", "r");
       if($handle) {
       while(($line = fgets($handle)) !== false) {
       if(startsWith($line, "DRIVERNAME=")) {
       echo substr($line, 11);
...
<div style="display: inline-block; width: 30%; background-color: #DFDFDF;">
  ...
  <?php
	$ccc = $c + 0;
       $c = 0;
       $handle = fopen("H3_entry_list.ini", "r");
       if($handle) {
       while(($line = fgets($handle)) !== false) {
       if(startsWith($line, "DRIVERNAME=")) {
       echo substr($line, 11);
...
<div style="display: inline-block; width: 50%; background-color: #DFDFDF;">
  ...
  <?php
       $c = 0;
       $handle = fopen("entry_list_2.ini", "r");
       if($handle) {
       $ball = 0;
       while(($line = fgets($handle)) !== false) {
       if(startsWith($line, "DRIVERNAME=")) {
